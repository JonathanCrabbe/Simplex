...
for cv in cv_list:
    for k in k_list:
        ...
        for explainer_name in explainer_names:
            ...
            latent_residual = np.sqrt(((latent_true - latent_approx) ** 2).mean()).item()
            output_residual = np.sqrt(((output_true - output_approx) ** 2).mean()).item()
            results_df = results_df.append({'explainer': explainer_name, 'k': k, 'cv': cv,
                                            'r2_latent': latent_r2, 'r2_output': output_r2,
